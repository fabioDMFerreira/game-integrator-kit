export default class World {
  ...
  updateVelocity() {
    if (this.Obj.Falling) {
      ...
      if (this.Obj.inclination.z) {
        this.Obj.addAccelarationX(
          -this.gravity * Math.sin(this.Obj.inclination.z)
        );
      }
      if (this.Obj.inclination.x) {
        this.Obj.addAccelarationZ(
          this.gravity * Math.sin(this.Obj.inclination.x)
        );
      }
    }
    ...
    for (let i = 0; i < this.objMovable.length; i++) {
      if (this.objMovable[i].Falling) {
        ...
        if (this.objMovable[i].inclination.z) {
          this.objMovable[i].addAccelarationX(
            -this.gravity * Math.sin(this.objMovable[i].inclination.z)
          );
        }
        if (this.objMovable[i].inclination.x) {
          this.objMovable[i].addAccelarationZ(
            this.gravity * Math.sin(this.objMovable[i].inclination.x)
          );
        }
      }
  ...
  moves(keyboard) {
    if (!this.Obj.Falling && this.keyboardStatus) {
      if (this.keyboard[87] && this.Obj.moveFlag.w) {
        // W
      ...
      if (this.keyboard[83] && this.Obj.moveFlag.s) {
        // S
        // this.Obj.addAccelarationZ(-0.4);
        this.Obj.addAccelarationZ(-Math.sin(Math.PI + this.anguloCamera) * 0.4);
        this.Obj.addAccelarationX(Math.cos(Math.PI + this.anguloCamera) * 0.4);
      ...
      if (this.keyboard[65] && this.Obj.moveFlag.a) {
        // A
        // this.Obj.addAccelarationX(0.4);
        this.Obj.addAccelarationZ(
          -Math.sin(Math.PI / 2 + this.anguloCamera) * 0.4
        );
        this.Obj.addAccelarationX(
          Math.cos(Math.PI / 2 + this.anguloCamera) * 0.4
        );
      ...
      if (this.keyboard[68] && this.Obj.moveFlag.d) {
        // D
        // this.Obj.addAccelarationX(-0.4);
        this.Obj.addAccelarationZ(
          Math.sin(Math.PI / 2 + this.anguloCamera) * 0.4
